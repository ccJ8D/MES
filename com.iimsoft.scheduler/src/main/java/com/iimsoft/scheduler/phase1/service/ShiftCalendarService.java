...
public class ShiftCalendarService {
    ...
    public ShiftCalendarService(int maxForwardSearchDays, boolean autoRollTemplate) {
    ...
    public void registerDailyTemplate(int workCenterId) {
        try {
            List<I_mom_work_center_shift> workCenterShifts = queryBL.createQueryBuilder(I_mom_work_center_shift.class)
                    .addEqualsFilter(I_mom_work_center_shift.COLUMNNAME_work_center_bo, workCenterId)
                    .create().list();
            if (wcTemplates.get(workCenterId) != null){
                return;
            }
            if (workCenterShifts.isEmpty()){
                ...
                List<I_mom_user_shift_definition> shiftDefinitions = queryBL.createQueryBuilder(I_mom_user_shift_definition.class)
                        .addEqualsFilter(I_mom_user_shift_definition.COLUMNNAME_user_shift_bo, workCenterShifts.get(0).getuser_shift_bo())
                        .create().list();
                List<DailyTemplate> templates = new ArrayList<>();
                for (I_mom_user_shift_definition shiftDefinition : shiftDefinitions) {
                    Timestamp s = shiftDefinition.getshift_begin();
                    Timestamp e = shiftDefinition.getshift_end();
                    templates.add(new DailyTemplate(s.toLocalDateTime().toLocalTime(), e.toLocalDateTime().toLocalTime()));
                }
                wcTemplates.put(workCenterId,templates);
            }
            clearCache(workCenterId);
