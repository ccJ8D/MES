...
public class SchedulingEngine {
    ...
    // 瓶颈保护时间（可配置）
    private final Duration BOTTLENECK_PROTECTION = Duration.ofHours(2);
    ...
    /**
     * bottleneckRouter: 表示被识别为瓶颈的 router_bo（工艺路线）
     */
    public ScheduleResult scheduleOrder(ShopOrder order, String bottleneckRouter) {
        if (order.isPullSystem()) {
            return backwardSchedule(order, bottleneckRouter);
        } else {
            return forwardSchedule(order, bottleneckRouter);
        }
    }
    
    private ScheduleResult forwardSchedule(ShopOrder order, String bottleneckRouter) {
        ...
        // 如果该订单的 router 与瓶颈 router 一致，则对该订单的每个步骤额外加保护时间，
        // 或者优先排在前面。这里实现为在计算 step duration 后增加 BOTTLENECK_PROTECTION。
        boolean isBottleneckRouter = order.getRouterBo() != null && order.getRouterBo().equals(bottleneckRouter);
        ...
        for (RouterStep step : steps) {
            ...
            if (isBottleneckRouter) {
                duration = duration.plus(BOTTLENECK_PROTECTION);
            }
            
            // 非瓶颈且需要安全库存的步骤加上 SAFETY_STOCK_BUFFER
            if (!isBottleneckRouter && requiresSafetyStock(step)) {
    ...
    private ScheduleResult backwardSchedule(ShopOrder order, String bottleneckRouter) {
        List<RouterStep> steps = loader.loadRouterSteps(order.getRouterBo());
    ...
    private Map<String, Duration> calculateStepDurations(ShopOrder order, List<RouterStep> steps) {
        ...
        for (RouterStep step : steps) {
            ...
            double learningFactor = 1.0 - (0.05 * Math.log10(Math.max(order.getQtyToBuild(), 1.0)));
